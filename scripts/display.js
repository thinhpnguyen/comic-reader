function display(mode, currentPage, files) {
  switch (mode) {
    case "single":
      break;
    case "double":
      displayDoubly(currentPage, files);
      break;
    case "conti":
...
function displayDoubly(currentPage, files) {
  // double page

  console.log(currentPage);
  //only display 1 page for the cover
  //and if the last page is odd then only display 1 page
  let k = currentPage.val === files.length - 1 || currentPage.val === 0 ? 0 : 1;
